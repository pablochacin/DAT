...
*/
public static synchronized int getNextAvailable(int fromPort,int range) throws NoSuchElementException{
	if ((fromPort < MIN_PORT_NUMBER) || (fromPort+range > MAX_PORT_NUMBER)) {
		throw new IllegalArgumentException ("Invalid port range: ["+ fromPort + ":"+ fromPort+range);
	}

	for (int i = fromPort; i < fromPort+range; i++) {
		if (available(i)) {
			return i;
		}
	}

	throw new NoSuchElementException ("Could not find an available port in the range [" + 
			                           fromPort+ ":" + (fromPort+range-1) + "]");
}
...
public static synchronized Set <Integer > getAvailablePorts(int fromPort, int toPort) {
